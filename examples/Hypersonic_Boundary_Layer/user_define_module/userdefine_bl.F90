...
! #endif
  ...
  subroutine udf_grid
    ...
    real(8) :: bl_thickness,dy1,dym,xmax,ymax,lz,lx
    real(8),allocatable :: xx(:),yy(:)

    integer :: i,in,j,jn,jbl,k,ii,jj,kk

    bl_thickness=0.1d0

    ymax=10.d0*bl_thickness

    jn=ja-50
    jbl=jn/2
    dym=bl_thickness/jbl
    dy1=0.1d0*dym

    allocate(yy(0:ja))

    yy(0:jn)=quatcosmesh(jn,dymin=dy1,ymax=3.d0*bl_thickness)

    ! call hyperb(ymin=dy1,ymax=5.d0*bl_thickness,y=yy(0:jn),jn=jn)

    ! yy(jbl:jn)=quatcosmesh(jn-jbl,dymin=yy(jbl)-yy(jbl-1),ymax=3.d0*bl_thickness)+yy(jbl)

    call spongstretch_2(ja-jn,ymax,yy(jn-2:ja))
